...
class DataBreachReportForm(forms.ModelForm):
    class Meta:
        ...
        fields = ('organization', 'incident_date', 'discovery_date', 'description', 'affected_data_subjects',
                  'data_types_affected', 'severity', 'potential_impact', 'remediation_steps',
                  'reported_to_authorities', 'date_reported_to_authorities',
                  'reported_to_data_subjects', 'date_reported_to_data_subjects')

class ComplianceAuditForm(forms.ModelForm):
    class Meta:
        model = ComplianceAudit
        fields = ('organization', 'auditor', 'audit_findings', 'recommendations', 'is_resolved', 'resolution_date')